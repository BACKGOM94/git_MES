...
function dw_scrollObj(wnId,lyrId,cntId,area,No){
    ...
    if(!this.write_scroll_bar(wnId,area,cntId,No)){
		return;
	}
...
dw_scrollObj.prototype.write_scroll_bar = function(content,where,text,No){
    ...
    where_obj.innerHTML += '<div id="track' + No + '" style="position: relative; float:right; top: 0; width: 11px; height: 100%; font-size: 0; z-index: 3; background: black;"><div id="drag_bar' + No + '" style="position: absolute; width: 100%; height: 14px; cursor: pointer; z-index: 4; background: yellow;"></div></div>';
    /* set text width */
    var scroll_width = document.getElementById('track' + No).offsetWidth;
    document.getElementById(text).style.width = content_width - scroll_width - 3 + 'px';
    return true;
}
