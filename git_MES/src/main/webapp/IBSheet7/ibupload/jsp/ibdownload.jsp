<%@ page contentType="text/html; charset=utf-8" pageEncoding="utf-8"%><%@ page import="java.io.*,java.util.*,java.net.URLEncoder, javax.servlet.*,org.apache.tools.zip.ZipEntry, org.apache.tools.zip.ZipOutputStream"%><%@ page import="org.apache.commons.fileupload.*"%><%@ page import="org.apache.commons.fileupload.disk.*"%><%@ page import="org.apache.commons.fileupload.servlet.*"%><%
	...
	//================================================================================
	String sServerName = request.getServerName().toString();
	System.out.println("IBDOWNLOAD ::: sServerName		= " + sServerName);
	String UploadRootDir;
	if("localhost".equals(sServerName)) {
		UploadRootDir = "D:/file_uploaded/";
	} else {
		//UploadRootDir = "/webapp/upload/";
		UploadRootDir = "/upload/";
	}
	System.out.println("IBDOWNLOAD ::: UploadRootDir	= " + UploadRootDir);
	...
	if (ContentType.indexOf("multipart/form-data") == -1) {
		...
		System.out.println("IBDOWNLOAD ::: DownloadFileURL = " + DownloadFileURL);
		System.out.println("IBDOWNLOAD ::: DownloadFileName after = " + DownloadFileName);
		...
		if (items.length == 1) {
			System.out.println("IBDOWNLOAD ::: 파일1개 선택");
			response.setContentType("application/octet-stream");
...
// 			item = item.substring(item.indexOf("/") + 1, item.length());
			...
			try {
				...
				if (new File(GetRealFilePathFromURLKey(UploadRootDir, item)).isFile()) {
					if (UserAgent.contains("MSIE") || UserAgent.contains("Trident")
						...
						//shkim 2016.11.11 개발서버,운영서버에서 chrome 브라우져로 한글파일 다운로드시 한글 깨짐 방지.
						if("localhost".equals(sServerName)) {
							DownloadFileName = new String(DownloadFileName.getBytes("utf-8"), "ISO-8859-1");
						} 
						response.setHeader("Content-Disposition", "attachment;filename=\"" + DownloadFileName + "\"");
					}
					System.out.println("IBDOWNLOAD ::: DownloadFileName before	= " + DownloadFileName);
					System.out.println("IBDOWNLOAD ::: UploadRootDir			= " + UploadRootDir);
					System.out.println("IBDOWNLOAD ::: item						= " + item);
			...
			System.out.println("IBDOWNLOAD ::: 파일다수 선택");
...
// 					item = item.substring(item.indexOf("/") + 1, item.length());
					...
					System.out.println("IBDOWNLOAD ::: DownloadFileURL before	= " + DownloadFileURL);
					System.out.println("IBDOWNLOAD ::: DownloadFileName before	= " + DownloadFileName);
					System.out.println("IBDOWNLOAD ::: UploadRootDir			= " + UploadRootDir);
					System.out.println("IBDOWNLOAD ::: item						= " + item);
					//====================================================================================================
					...
					if (new File(GetRealFilePathFromURLKey(UploadRootDir, DownloadFileURL)).isFile()) {
						...
						FileInputStream fis = new FileInputStream(DownloadFileURL);
						BufferedInputStream bis = new BufferedInputStream(fis);
...
%><%!//====================================================================================================
	...
	String GetRealFilePathFromURLKey(String UploadRootDir, String UrlKey) {
		System.out.println("IBDOWNLOAD ::: GetRealFilePathFromURLKey ::: UploadRootDir	= " + UploadRootDir);
		System.out.println("IBDOWNLOAD ::: GetRealFilePathFromURLKey ::: UrlKey			= " + UrlKey);
